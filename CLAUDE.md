...
8. **Une issue = une branche = une PR** : jamais deux sujets dans la même PR. Branche nommée `claude/<numéro d'issue>-<slug>` depuis `origin/main`, corps de PR rempli depuis `.github/pull_request_template.md` (la section « Docs (required) » n'est pas décorative), et un `Closes #<issue>` — **le mot-clé se met en anglais** (`Closes`/`Fixes`/`Resolves`) : GitHub ignore « Ferme #123 », l'issue reste ouverte après le merge et il faut la fermer à la main. `main` est squashée : deux commits sans rapport sur une même branche produisent **un seul** commit qui les mélange, et l'historique perd le lien correctif↔issue que tout le repo tient par ailleurs (`type(scope) : titre (#issue) (#PR)`). Un travail qui ne se rattache à aucune issue — un rapport d'audit, une convention comme celle-ci — garde sa PR autonome plutôt que de s'ajouter à celle d'une issue
