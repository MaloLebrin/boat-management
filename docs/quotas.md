...
- L'**import CSV** de l'historique d'entretien (`/settings/import`) est réservé au plan Entreprise (#715) et, dans ce plan, aux **admins** (capability `import.run`). Capacité de tier pure (`PLAN_LIMITS[...].canImport`, jamais les quotas effectifs) : aucun module ni add-on ne l'accorde. `QuotaService.assertCanImport` garde les trois routes qui agissent (`preview`, `confirm`, `cancel`), avant la capability, avec le flash `flash.quota.importExceeded` et la redirection vers `/settings/billing`. L'écran lui-même reste ouvert à qui n'a que les **exports** — `canExport`, ouvert dès Pro et à tous les rôles, sur la même page — et reçoit la prop `canImport` qui décide de sa section d'import ; il ne redirige que si ni l'un ni l'autre n'est accordé.
