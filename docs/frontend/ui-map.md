...
- Pages : `inertia/pages/marketing/{home,pricing,about,contact,guide,simulator,simulator_share,privacy,terms,sales_terms,legal_notice,diagnosis_ai,parts_ai}.vue` — rendues par `MarketingController` (routes locale-préfixées `/en`, `/fr`, voir `start/routes/marketing.ts`), layout `inertia/layouts/public.vue` ; `diagnosis_ai.vue` est rendue par `PublicDiagnosisController` (#602) et `parts_ai.vue` par `PublicPartSearchController` (#634 Phase 2)
