...
- Page: `inertia/pages/dashboard.vue` — orchestration seule : ordre des blocs et répartition en colonnes, aucun rendu métier inline (#828, #832)
...
- Source backend: `HomeController.index` → `DashboardService`, `DashboardAttentionService`, `DashboardFleetActivityService`, `BoatReservationService.listUpcomingForOrg`, `BudgetService.getOrgSpendSummary` — règles dans `docs/domain/dashboard.md`
...
- Props différées : `prop === undefined` = pas encore arrivée → `BaseSkeleton` (`DashboardActivityCard`, `DashboardSpendCard`) ; les gardes de rôle / module omettent la prop côté serveur, le front lit `canViewSpend` et la présence de `upcomingReservations`.
- Composants (`inertia/components/dashboard/`) :
  - En-tête: `DashboardHeader.vue` — `h1` + date du jour (`formatDateLong`, calculée en `onMounted` pour éviter un mismatch d'hydratation) + slot `actions` (menu « + Créer », `DashboardQuickAddActions.vue`)
  ...
  - Ports: `PortDashboardCard.vue` — réservée aux plans avec `canManagePorts` (#604)
- Tests : `dashboard_page_layout.spec.ts` (ordre, colonnes, gardes), `dashboard_attention_card`, `dashboard_stats_grid`, `dashboard_at_sea_card`, `dashboard_upcoming_reservations_card`, `dashboard_activity_card`, `dashboard_spend_card`, `dashboard_ai_panel_analysed_at`, `dashboard_header`, `dashboard_boats_card`, `dashboard_ports_card_plan` ; fonctionnel `tests/functional/dashboard/dashboard_props.spec.ts` (gardes, groupes différés) ; navigateur `dashboard_card_links.spec.ts` (KPI, À traiter → planning), `mobile_field.spec.ts` (non-débordement, cartes bateaux), `touch_targets.spec.ts`
