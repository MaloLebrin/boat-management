...
- Slugs de la page tarifs (#475) : `/en/pricing` en anglais, `/fr/tarifs` en français ; `/en/tarifs` ne subsiste que comme redirection 301 (`marketing.en.pricing_legacy`). Un lien vers cette page ne se construit **jamais** en interpolant la locale (`/${locale}/tarifs`) — il passe par `pricingPath(locale)` / `PRICING_PATHS` (`shared/helpers/locale_path.ts`), utilisés par `AppHeader.vue`, `AppHeaderMobileDrawer.vue`, le footer de `layouts/public.vue`, `home/{HomeFaqCtaSection,HomeDemoSection}.vue`, le `ctaHref` de l'offre modulaire (`MarketingController`) et le canonical/hreflang de `pricing.vue`. Le sélecteur de langue s'appuie sur l'alias `/pricing` ↔ `/tarifs` de `LOCALIZED_PATH_ALIASES`
