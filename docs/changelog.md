...
Conséquences : pas de précache, pas de consultation hors-ligne, `offline.html` jamais servie, toast « prête hors-ligne » jamais déclenché. **L'installation, en revanche, fonctionne** — Chrome n'exige plus de service worker pour déclencher `beforeinstallprompt`, et #459 rapporte que le bouton s'affiche bien ; l'app est donc installable, mais sans aucune capacité hors-ligne une fois installée. **La file d'attente hors-ligne continue elle aussi de fonctionner** : elle est en JS de page + IndexedDB, indépendante du service worker. Aucun test ne couvrait le service worker — les specs Vitest testent les composables, pas Workbox — d'où l'absence de signal.
