...
| Bouton « Installer l'application » (`usePwaInstall`, dans `AsideMenu.vue`) | ✅ **S'affiche** — Chrome n'exige plus de service worker pour déclencher `beforeinstallprompt`, le manifest suffit. Confirmé par la campagne de test du 03/08/2026 (#459), qui l'a vu à l'écran. L'app est donc installable aujourd'hui ; elle n'a simplement aucune capacité hors-ligne une fois installée. _(#459 signale au passage que le libellé affiche la clé brute « pwa.install ».)_ |
...
**Effort : 1 à 2 jours**, dont la moitié en vérification sur build réel. **À faire avant toute autre chose** — sans service worker il n'y a ni cache hors-ligne ni Web Push. (L'installation, elle, fonctionne déjà : voir le tableau ci-dessus.)
...
- **Entonnoir iOS** : Web Push exige une PWA installée. Sur Android l'entonnoir existe déjà et fonctionne (`beforeinstallprompt` se déclenche, voir §0) ; c'est sur Safari qu'il manque, l'événement n'y existant pas — `canInstall` y sera toujours `false`, il faut un composant d'aide dédié avec les instructions « Partager → Sur l'écran d'accueil ».
