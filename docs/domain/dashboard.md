...
- **Routes** : `PUT /dashboard/layout` (`{ order: { main, side }, hidden }`, ids du registre, `distinct`) et `DELETE /dashboard/layout` (retour au défaut) → `redirect().back()` + flash. La modale « Personnaliser » (en-tête) porte un brouillon local : interrupteur par widget, flèches haut/bas, « Réinitialiser » / « Enregistrer ».
