...
| -------------------------- | ----------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------- |
| Organisation               | `members.view`, `invitations.view`, `audit_log.view`                                      | `members.manage`, `invitations.manage`, `ai.configure`, `branding.configure`, `organization.manage`, `import.run` |
...
`organization.manage` garde le renommage de l'organisation (`PUT /settings/org`) depuis #761 : admin seul, **aligné sur `branding.configure` et non sur `members.view`**. Les deux écrivent ce que les clients de l'exploitant lisent sur ses documents — la raison sociale est reprise sur les factures émises, les contrats de location et les carnets d'entretien. Jusque-là la route n'appelait aucune policy du tout, et tout membre authentifié renommait l'organisation ; `/settings/members` et `/settings/org` ne gardaient pas davantage leur lecture, alors que `SettingsShell` n'en affiche les onglets qu'à `members.view` (le contrat n'existait que côté front). Deux frontières distinctes en résultent : **ouvrir** ces deux écrans est `members.view` (admin + member), **renommer** est `organization.manage`. Conséquence assumée : un `member` ne renomme plus l'organisation, alors qu'il pouvait — `SettingsOrgTab` lui rend le nom en lecture seule plutôt qu'un formulaire que le backend refuse.

`import.run` garde l'import CSV (`/settings/import`) depuis #715 : admin seul, **aligné sur `maintenance.delete` et non sur `maintenance.create`**. L'import écrit en masse dans l'historique d'entretien, que seul un admin peut ensuite corriger — un `mechanic` pouvait jusque-là écrire un historique qu'il ne pouvait pas défaire, et un `boat_owner`, qui n'a aucune capability, en faisait autant. S'y ajoute une garde de plan, vérifiée **avant** la capability : sur une organisation qui n'a aucun import (Starter), l'upsell vers la facturation dit plus qu'un 403 de rôle. Le palier dépend du **type** (`CSV_IMPORT_PLAN_FLAGS`) : l'historique d'entretien reste en Entreprise (`PlanQuotas.canImport`), les dépenses du budget s'importent dès Pro (`PlanQuotas.canImportExpenses`) — `preview` et `confirm` re-vérifient ce palier une fois le type connu. Les **exports** de la même page ne sont pas concernés — ils s'arrêtent à `canExport` (Pro et Entreprise, tous rôles), et c'est pourquoi `show` reste ouvert avec une prop `canImport` plutôt que fermé.
