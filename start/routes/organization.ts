...
router
  .group(() => {
    ...
    router.post('organization/invitations', [controllers.OrganizationInvitations, 'store']).as('organization.invitations.store')
    router.delete('organization/invitations/:id', [controllers.OrganizationInvitations, 'destroy']).as('organization.invitations.destroy')
  })
  .use(middleware.auth())
