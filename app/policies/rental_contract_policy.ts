...
export default class RentalContractPolicy extends OrgScopedPolicy {
  async view(user: User, reservation: BoatReservation): Promise<AuthorizerResponse> {
    return this.sameOrg(user, reservation) && (await this.can(user, 'rentalContracts.view'))
  }

  async create(user: User, reservation: BoatReservation): Promise<AuthorizerResponse> {
    return this.sameOrg(user, reservation) && (await this.can(user, 'rentalContracts.create'))
  }

  async edit(user: User, reservation: BoatReservation): Promise<AuthorizerResponse> {
    return this.sameOrg(user, reservation) && (await this.can(user, 'rentalContracts.edit'))
  }

  async delete(user: User, reservation: BoatReservation): Promise<AuthorizerResponse> {
    return this.sameOrg(user, reservation) && (await this.can(user, 'rentalContracts.delete'))
  }
}
