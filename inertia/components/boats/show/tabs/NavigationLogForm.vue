...
<template>
  <div class="rounded-lg border border-border bg-surface-elevated p-6 space-y-4">
    ...
    <Form
      :action="{ url: `/boats/${boat.id}/navigation-logs`, method: 'post' }"
    ...
    >
      <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
        ...
        <div class="sm:col-span-2">
          ...
          />
          <p v-if="errors.departedAt" class="mt-1 text-xs text-danger">{{ errors.departedAt }}</p>
        </div>
        ...
        <div>
          ...
          <input
            ...
            class="w-full rounded-md border border-border bg-surface px-3 py-2 text-sm text-fg focus:outline-none focus:ring-2 focus:ring-brand"
          />
          <p v-if="errors.engineHoursStart" class="mt-1 text-xs text-danger">
            {{ errors.engineHoursStart }}
          </p>
        </div>
