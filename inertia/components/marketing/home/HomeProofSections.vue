<!--
  ...
  Rôle d'origine : ensemble de sections de preuve (stats, logos, témoignages) d'une ancienne itération de la home.
...
const props = defineProps<{
  ...
  testimonials: {
    title: string
    items: Array<{ quote: string; author: string; role: string; fleet: string; since: string }>
  }
  comparisonTable: {
    title: string
    subtitle: string
  ...
  blog: {
    ...
    subtitle: string
    cta: string
    articles: Array<{ cat: string; title: string; meta: string }>
  }
}>()
...
<template>
    ...
    <div class="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
      ...
      >
        ...
        <div class="mt-4">
          ...
          <p class="mt-2 text-xs text-fg-subtle border-t border-bone pt-2">
            {{ item.fleet }} · {{ item.since }}
          </p>
        </div>
      </div>
    </div>
  ...
  >
    <div class="mb-8 text-center">
      ...
      <h2 class="mt-2 font-display text-3xl leading-tight italic text-fg lg:text-4xl">
      ...
      <table class="w-full text-sm">
        <thead class="bg-bone/40 text-xs font-semibold uppercase tracking-wider text-fg-subtle">
          <tr>
            ...
            <th class="px-4 py-3 text-center bg-brand/8 text-brand font-bold">
              {{ comparisonTable.cols.fleetai }}
            </th>
          </tr>
        </thead>
        <tbody>
          <tr
            v-for="(row, i) in comparisonTable.rows"
            :key="row"
            :class="i % 2 === 0 ? 'bg-cream' : 'bg-paper'"
          >
            <td class="px-4 py-3 font-medium text-fg">{{ row }}</td>
            <td class="px-4 py-3 text-center">
              <span class="text-fg-muted">{{ getComparisonVal(row, 'excel') }}</span>
            </td>
            <td class="px-4 py-3 text-center">
              <span class="text-fg-subtle">{{ getComparisonVal(row, 'paper') }}</span>
            </td>
            <td class="px-4 py-3 text-center bg-brand/5">
              <span class="font-semibold text-mint-700">{{
                getComparisonVal(row, 'fleetai')
              }}</span>
            </td>
          </tr>
        </tbody>
  ...
  <section :ref="securityEl" class="mt-14 reveal" :class="{ visible: securityVisible }">
    <BaseCard padded>
      ...
      <div class="grid gap-4 md:grid-cols-2">
        ...
        >
          <div
            class="flex h-10 w-10 items-center justify-center rounded-xl bg-navy-900/8 text-xl mb-3"
          >
            {{ s.icon }}
          </div>
  ...
  >
    <div class="mb-8 flex items-end justify-between">
      <div>
        ...
        <h2 class="mt-1 font-display text-3xl leading-tight italic text-fg lg:text-4xl">
          {{ blog.title }}
        </h2>
      </div>
    ...
    <div class="grid gap-5 sm:grid-cols-3">
      ...
      >
        <div
          class="flex h-32 items-center justify-center bg-bone/40 text-xs text-fg-subtle group-hover:scale-105 transition-transform duration-500"
        >
          image article
        </div>
        <div class="p-4">
          <span
            class="inline-block rounded-full bg-paper px-2 py-0.5 text-xs font-medium text-fg-muted border border-bone"
            >{{ article.cat }}</span
          >
