...
function selectWear(value: SimulatorWearLevel) {
  emit('update:modelValue', { ...props.modelValue, riggingWear: value })
}

const canProceed = computed(() => Boolean(props.modelValue.riggingWear))
</script>
      ...
      <div class="space-y-3">
        <button
          ...
          :class="[
            ...
            modelValue.riggingWear === opt.value
        ...
        >
          <span
            :class="[
              ...
              modelValue.riggingWear === opt.value ? 'text-coral-700' : 'text-fg',
            ]"
          >
            {{ t(opt.labelKey) }}
          </span>
