...
| --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------- |
| `GradientMeshCanvas.vue`    | Dégradé maillé de hero, technique stripe.com : **shader WebGL** (simplex Ashima + FBM + domain warping, `mesh_gradient_webgl.ts`), repli **blobs canvas 2D** (`mesh_gradient_2d.ts`) puis fond CSS. Props `variant` (`navy`/`sunset`/`ocean`/`dawn` — clair pour fond cream) + `intensity`. | `HomeHeroSection`, `PricingHeroSection`, `AboutHeroSection` |
...
| `PortsMapCanvas.vue`        | Carte du monde pointillée (grille hexagonale précalculée en offscreen, `ports_map_renderer.ts` + `world_map_data.ts`) avec arcs-« comètes » entre ports et ports pulsants — inspirée du globe Stripe, à plat. Props `variant` (`dark`/`light`) + `intensity`.                               | `HomeStatsBandSection`, `ContactHeroSection`                |

**Garde-fous communs** (mutualisés dans `use_canvas_lifecycle.ts` pour le mesh et la carte) : `IntersectionObserver` (pause hors écran) + `visibilitychange` (pause onglet caché), `devicePixelRatio` plafonné (2 en 2D, 1.5 en WebGL), `getContext` en `try/catch` (fallback si canvas indisponible, ex. jsdom), frame statique immédiate (couvre `prefers-reduced-motion`), nettoyage complet des listeners/rAF en `onUnmounted`. Le moteur WebGL gère en plus `webglcontextlost`/`webglcontextrestored`.
...
| `use_canvas_lifecycle.ts` | Cycle de vie commun des canvas décoratifs : frame statique immédiate (reduced-motion), boucle rAF pausée hors écran/onglet caché, resize, cleanup. Prend une factory `(el) => renderer \| null` (null → repli CSS). | `GradientMeshCanvas`, `PortsMapCanvas`             |
...
| `.section-diagonal-band` / `.section-diagonal-top` | Bande/arête diagonale « Stripe » via `clip-path: polygon(...)` sur un calque de fond `inset-0` (pas de transform) — le canvas/contenu à l'intérieur reste dans son flux normal, aucun contre-transform ni `scale` de compensation à deviner (un `skewY` imbriqué + contre-skew avait été essayé puis abandonné : le facteur de `scale` nécessaire dépend du ratio largeur/hauteur de la section et ne peut pas être fixé une fois pour toutes, ce qui laissait des coins non couverts). `-band` : diagonale haut+bas. `-top` : diagonale en haut, bas plat. | —               | `HomeStatsBandSection` (bande navy), `HomeFinalCtaSection` (arête haute) |
| `.task-scroll`                                     | Défilement vertical continu du panneau « À venir » du mock hero (liste dupliquée, boucle `translateY(-50%)` sans couture). Pause au survol.                                                                                                                                                                                                                                                                                                                                                                                                                 | `taskScroll`    | `HomeMockUpcomingTasks` (mock hero)                                      |
