...
| `.section-skew` + `.section-skew-content` | Bande diagonale « Stripe » : un calque de fond absolu incliné (`skewY(-6deg)`, insets verticaux en `vw` ≥ 5.25vw d'excursion), le canvas/motif intérieur contre-skewé + `scale(1.15)`, le texte reste un frère droit. Transforms statiques → rien à couper sous reduced-motion. | —               | `HomeStatsBandSection` (bande navy), `HomeFinalCtaSection` (arête haute) |
